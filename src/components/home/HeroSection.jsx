...
const HeroSection = () => {
  ...
  return (
    <section
      ref={sectionRef}
      className="relative h-screen min-h-[760px] overflow-hidden bg-gray-950 select-none"
    >
      {/* ══════ BACKGROUND IMAGES ══════ */}
      {slides.map((slide, index) => (
        <div
          key={index}
          className="absolute inset-0 transition-opacity duration-[1400ms] ease-[cubic-bezier(.4,0,.2,1)]"
          style={{
        ...
        >
          ...
          >
            <img
              src={slide.image}
              alt={slide.title}
              className={`w-full h-full object-cover will-change-transform transition-transform duration-[9000ms] ease-out ${
      ...
      {/* ══════ OVERLAYS ══════ */}
      <div className="absolute inset-0 z-[3] bg-gradient-to-b from-gray-950/70 via-gray-950/25 to-gray-950" />
      <div className="absolute inset-0 z-[3] bg-gradient-to-r from-gray-950/80 via-gray-950/30 to-transparent" />
          ...
          >
            ...
            <div
              className={`rounded-full overflow-hidden transition-all duration-500 ${
                i === current ? "w-1.5 h-12 bg-white/15" : "w-1 h-5 bg-white/15 group-hover:bg-white/25"
              }`}
            >
              {i === current && (
                <div
                  className="w-full bg-emerald-400 rounded-full transition-none"
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-20 xl:px-24 w-full">
          <div className="grid lg:grid-cols-[1fr_420px] gap-10 xl:gap-16 items-center">
            ...
            <div>
              ...
              >
                <span className="flex items-center gap-2 bg-white/[0.07] backdrop-blur-xl border border-white/[0.1] rounded-full pl-3 pr-4 py-1.5">
                ...
                <span className="text-white/50 text-xs font-medium flex items-center gap-1.5">
                  <FaMapMarkerAlt className="text-[10px] text-emerald-400/70" />
              ...
              <h1
                key={`title-${current}`}
                className="mb-6 animate-[heroFadeUp_0.75s_ease-out]"
              >
                {slides[current].title.split(" ").map((word, wi, arr) => {
                  if (wi < Math.min(2, arr.length - 1)) {
                    return (
                      <span
                        key={wi}
                        className="text-[2.75rem] sm:text-6xl lg:text-[4.25rem] xl:text-[5rem] font-extrabold text-white leading-[1.1] tracking-tight"
                      >
                        {word}{" "}
                      </span>
                    );
                  ...
                  return (
                    <span
                      key={wi}
                      className="hero-text-outline text-[2.75rem] sm:text-6xl lg:text-[4.25rem] xl:text-[5rem] font-extrabold leading-[1.1] tracking-tight"
                    >
              ...
              <div className="flex items-center gap-3 mb-6 animate-[heroFadeUp_0.75s_0.08s_ease-out_both]">
                <div className="h-[3px] w-12 rounded-full bg-gradient-to-r from-emerald-400 to-teal-400" />
              ...
              <p
                key={`sub-${current}`}
                className="text-base sm:text-lg text-white/50 max-w-md leading-relaxed mb-10 animate-[heroFadeUp_0.8s_0.15s_ease-out_both]"
              >
              ...
              <div className="flex flex-wrap items-center gap-4 mb-10 animate-[heroFadeUp_0.8s_0.25s_ease-out_both]">
                <Link
                  to="/packages"
                  className="group relative bg-emerald-500 text-white pl-7 pr-5 py-3.5 rounded-full font-semibold text-sm overflow-hidden transition-all duration-300 hover:shadow-[0_8px_32px_rgba(16,185,129,.3)] hover:-translate-y-0.5 active:scale-[.97]"
                >
                  <span className="relative z-10 flex items-center gap-3">
                    ...
                    <span className="flex items-center justify-center w-8 h-8 rounded-full bg-white/20 group-hover:bg-white/30 transition-colors">
                      <HiArrowLongRight className="text-lg group-hover:translate-x-0.5 transition-transform" />
                    </span>
                  </span>
                ...
                <Link
                  to="/destinations"
                  className="group flex items-center gap-2.5 text-white/80 px-6 py-3.5 rounded-full border border-white/[0.1] bg-white/[0.04] backdrop-blur-xl hover:bg-white/[0.08] hover:border-white/20 transition-all duration-300 font-medium text-sm"
                >
              ...
              >
                <div className="relative flex items-center bg-white/[0.06] backdrop-blur-2xl border border-white/[0.08] rounded-2xl pl-4 pr-1.5 py-1.5 hover:border-white/15 focus-within:border-emerald-500/30 transition-colors shadow-[0_4px_24px_rgba(0,0,0,.2)]">
                  ...
                  <input
                    ...
                    className="flex-1 bg-transparent text-white placeholder-white/25 focus:outline-none text-sm font-medium py-2"
                  />
                  <button
                    type="submit"
                    className="shrink-0 bg-emerald-500 hover:bg-emerald-400 text-white rounded-xl font-semibold px-5 py-2.5 text-xs transition-all duration-300 hover:shadow-lg hover:shadow-emerald-500/20"
                  >
                    Search
                  </button>
                  <SearchSuggestions
            ...
            <div className="hidden lg:block relative h-[480px] animate-[heroFadeUp_1s_0.2s_ease-out_both]">
              {/* Background card (next slide) */}
              <div className="absolute top-6 left-6 right-0 bottom-0 rounded-[28px] overflow-hidden border border-white/[0.06] opacity-40 blur-[1px]">
                <img
              ...
              <div
                key={`card-${current}`}
                className="relative w-full h-full rounded-[28px] overflow-hidden shadow-[0_25px_60px_rgba(0,0,0,.5)] border border-white/[0.1] animate-[heroCardIn_0.7s_ease-out]"
              >
                <img
      ...
      <div className="absolute bottom-0 right-0 z-20 w-auto max-w-max">
        <div className="bg-gradient-to-t from-gray-950/90 to-transparent pt-16 pb-0 pr-4 sm:pr-6 lg:pr-20 xl:pr-24">
          <div className="bg-white/[0.04] backdrop-blur-2xl border border-white/[0.08] rounded-tl-2xl rounded-tr-2xl px-6 sm:px-8 py-4">
